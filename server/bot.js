...
function getMenu() {
  return [
    [
      {
        text: "Play Now! 🎡",
        web_app: {
          url: "https://3601cb7c-30f1-4bb0-afc7-6bc1e2d23b08-00-15aostps5pm8m.sisko.replit.dev:3000/",
        },
      },
    ],
    [
    ...
    [
      ...
      { text: "Community 👥", url: "https://t.me/startrenew_bot" },
    ],
  ];
}
...
bot.start((ctx) => {
  ...
  if (refId) {
    ...
    axios
      .post(`${process.env.BASE_API_URL}referral`, {
      ...
      .then(() => {
        ctx.reply(`You invited by ${refId} to join the RabbitPunk.`);
        bot.telegram.sendMessage(
          (chat_id = Number(refId)),
          (text = `🪼 You invited your friend <a href="tg://user?id=${ctx.message.from.id}">${ctx.message.from.first_name}</a>! 🎉\n👀 Check your referrals by /referral`),
          {
            parse_mode: "HTML",
          },
        );
      ...
      .catch((error) => {
        ...
        ctx.reply(`Error for inviting ${refId} to join the RabbitPunk Bot.`);
      });
  }

  ctx.reply(
    `<b>Hi, dear <a href="tg://user?id=${ctx.message.from.id}">${ctx.message.from.first_name}</a>! This is RabbitPunk 👋!</b> \n
<b>Tap on the coin and watch your RabbitPunk grow.</b>`,
    {
      reply_markup: {
...
bot.command("referral", (ctx) => {
  let referrals = 0;
  axios
    .get(`${process.env.BASE_API_URL}user/${ctx.from.id}`)
    .then((response) => {
    ...
    .finally(() => {
      ctx.replyWithHTML(
        `<b>RabbitPunk, Invite your friends and get bonuses for each invited friend! 🎁
\n🎗Referral link: </b><code>https://t.me/startrenew_bot?start=${ctx.message.from.id}</code>
...
bot.action("referral", (ctx) => {
  ...
  axios
    ...
    .finally(() => {
      ctx.replyWithHTML(
        `<b>RabbitPunk, Invite your friends and get bonuses for each invited friend! 🎁
\n🎗Referral link: </b><code>https://t.me/startrenew_bot?start=${ctx.from.id}</code>
    \n<b>Your referrals : ${referrals}</b>`,
        {
          reply_markup: {
            inline_keyboard: [
...
bot.action("show_profile", (ctx) => {
  ...
  // get user rank
  axios
    .get(`${process.env.BASE_API_URL}user/${ctx.from.id}/get-rank`)
    .then((response) => {
...
bot.action("leaderboard", (ctx) => {
  ctx.deleteMessage();
  axios.get(`${process.env.BASE_API_URL}leaderboard`).then((response) => {
    let li = "";
