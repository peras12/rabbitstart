...
app.post("/api/user/:id/add-point", async (req, res) => {
  ...
  try {
    ...
    await database.ref("users/" + userId).update({
      points: userData.points + newScore,
      lastActive: new Date().toISOString(),
      limit: userData.limit - newScore,
    });
    return res
      .status(200)
      .send({ message: "Score and lastActive updated successfully" });
  } catch (error) {
    return res
      .status(500)
      .send({ error: "Error updating score", details: error });
  }
});
  ...
  userRef
    ...
    .then((snapshot) => {
      if (snapshot.exists()) {
        ...
        return res
          .status(200)
          .json({ userId: snapshot.val().userId, ...snapshot.val() });
      } else {
...
app.post("/api/withdraw", async (req, res) => {
  ...
  const message = await bot.telegram.sendMessage(
    userId,
    `<b>🛂 Project Info</b>\n\n🪼 Receiver: <b>${userAddress}</b>\n💎 Points: <b>${points}</b>\n\n<b>⏰ We invite you to our official website Rabbitworld.com to get information about our project and review our roadmap. If you wish, you can access detailed information such as the fund balance and reward types on our project's official funding sites from the Kickstarter LINK / Startup Project LINK addresses.</b>\n\n<b>Support : @Y-T-Support</b>`,
    {
      parse_mode: "HTML",
    },
  );
...
app.post("/api/change-skin", async (req, res) => {
  ...
  try {
    ...
    res.status(200).send({ message: "SkinID updated muccessfully" });
  } catch (error) {
    res.status(500).send({ error: "Error updating skinID", details: error });
  }
});
...
app.post("/api/referral", async (req, res) => {
  ...
  try {
    ...
    const refferalsInfo =
      referrerUserSnapshot.val().referralsInfo === "null"
        ? [newUserInfo]
        : [...referrerUserSnapshot.val().referralsInfo, newUserInfo];
    ...
    return res.status(500).send({
      error: "User inveted before or internal server error",
      details: error,
    });
  }
});
...
app.get("/api/user/:id/get-rank", async (req, res) => {
  ...
  try {
    ...
    const targetUserSnapshot = await database
      .ref("users/" + userId)
      .once("value");
    const targetUserData = targetUserSnapshot.val();

    // Get all users' data
    const allUsersSnapshot = await database
      .ref("users")
      .orderByChild("points")
      .once("value");
    ...
    const targetUserIndex = usersArray.findIndex(
...
app.get("/api/leaderboard", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .send({ error: "Error getting leaderboard", details: error });
  }
});
